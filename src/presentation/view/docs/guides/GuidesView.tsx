    ...
    <div className="max-w-7xl space-y-16 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-fade-in-up">
        <div className="space-y-2">
            <h1 className="heading-display text-foreground">Integration Guides</h1>
            <p className="body-lg text-muted max-w-2xl leading-relaxed">
                Master the Payment Kita ecosystem with our deep-dive tutorials and architectural patterns.
            </p>
        </div>
        <div className="px-5 py-3 rounded-2xl bg-primary/5 border border-primary/10 flex items-center gap-3 backdrop-blur-sm shadow-glow-purple/5">
            ...
            <span className="text-sm font-bold text-primary italic">Updates every week</span>
        </div>
      </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12">
            <div className="flex-1 space-y-8">
              <div className="flex items-center gap-3 text-primary">
                ...
                <span className="font-bold uppercase tracking-[0.3em] text-[10px]">Featured Masterclass</span>
              </div>
              <div className="space-y-6">
                <h3 className="heading-display text-foreground leading-[1.1]">Hosted Checkout Integration</h3>
                <p className="body-lg text-muted leading-relaxed">
                  The most seamless way to accept crypto. Deploy a world-class checkout experience in under 10 minutes with just a few lines of code.
                </p>
              </div>
              <div className="flex items-center gap-8">
                ...
                <Badge variant="warning" className="px-4 py-1.5 rounded-xl border-warning/20 text-xs shadow-glow-orange/5">Intermediate</Badge>
              </div>
              <Button size="lg" className="h-14 px-10 rounded-2xl text-lg font-bold shadow-glow-purple hover:scale-105 transition-transform">
                <Link href="/docs/guides/hosted-checkout" className="flex items-center gap-3">
                  Start Learning
...
window.location.href = session.url;`}
      ...
      <section className="space-y-8 pt-12 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
        <div className="text-center space-y-2">
            ...
            <p className="body-lg text-muted">Explore more ways to build with Payment Kita.</p>
        </div>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {[
            { title: 'API Docs', desc: 'Full reference', href: '/docs/api', icon: Code, color: 'text-primary' },
            { title: 'Community', desc: 'Join Discord', href: 'https://discord.gg/payment-kita', icon: Layout, color: 'text-accent-blue' },
            { title: 'Support', desc: 'Get help', href: 'mailto:support@payment-kita.com', icon: HelpCircle, color: 'text-accent-green' },
            { title: 'Masterclasses', desc: 'Video tutorials', href: '#', icon: Sparkles, color: 'text-warning' }
            ].map((res) => (
